...
| `type` | No | One of: `program`, `academic`, `excursion`, `holiday`, `orientation`, `mundial`. Defaults to `academic`. Each gets a distinct color. `program` carries the most prominent visual (Parchment-orange + Pep Orange + ★) and should be used for Director-curated entries like arrival days, asados, faculty visits, and other program-office-organized moments. `mundial` (added 2026-06-01) is for World Cup games — it renders a fun championship-gold card with a ⚽ soccer ball next to the title, and shows up wherever the event's `visibility` directs (use `visibility: week` to keep games in the Weekly Overview only). **Argentina game days get extra flair (added 2026-06-11):** if a `mundial` row is dated for the current day **and its `title` contains the word "Argentina"** (e.g. `Mundial: 🇦🇷 Argentina vs Austria 🇦🇹`), the Today tab dresses up in the albiceleste — a celeste greeting strip with the flag's sun and falling papelitos, plus a festive game tile showing the matchup and a live "Arranca en…" countdown to kickoff (read from the row's `start_time`). It's automatic: just title the game with "Argentina" and fill in `start_time`. Other countries' games render as the normal championship-gold mundial card with no extra treatment. The legacy `milestone` type was retired on 2026-05-11b — its visual was reassigned to `program`; rows still typed `milestone` in the sheet render as Academic cards until relabeled. |
