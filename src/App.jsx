...
const DEFAULT_DATA = {
  ...
  finals_window_start: "2026-07-01",
  finals_window_end: "2026-07-01",
  classes: [
...
// Cohort-wide "finals season" gate for the Today-tab Good-Luck mask
// (the greeting-strip skin + <FinalsGoodLuckTile>). True when today
// falls inside the program-wide finals window from Settings, inclusive
// on both ends. Deliberately independent of <shouldShowFinalsUI>: that
// gate is per-student (needs personalization + assigned/upcoming finals)
// and drives the practical finals tile/card; this morale treatment is
// for the whole cohort — everyone has finals — and reads straight off
// the window the Director controls in the sheet, so it turns on the day
// finals start and auto-retires the day after they end with no redeploy.
// YYYY-MM-DD strings compare correctly lexicographically. end_date is
// optional and defaults to start (a single-day Summer window).
function isFinalsSeason(data, todayStr) {
  const start = data.finals_window_start || "";
  const end = data.finals_window_end || start;
...
function AddToCalendarButton({ onClick }) {
  return (
    ...
    >
      <CalendarPlusIcon size={15} color={C.ocean} />
    </button>
  );
}
...
// Finals-week confetti — same drifting motion as <Papelitos> (and the
// same motion-safe .bap-papelitos / bap-papel-fall CSS), but in an
// academic-celebration palette (BAP blue / Pep gold accent / white)
// so it reads as "good luck on exams," not as the albiceleste.
const FINALS_CONFETTI = [
  { left: "5%",  w: 6, h: 9,  color: "#64B5F6", delay: "0s",   dur: "5.4s" },
  { left: "16%", w: 5, h: 8,  color: "#FFFFFF", delay: "1.9s", dur: "6.2s" },
  { left: "27%", w: 7, h: 10, color: "#F6B40E", delay: "0.7s", dur: "4.9s" },
  { left: "38%", w: 5, h: 7,  color: "#FFFFFF", delay: "2.9s", dur: "5.7s" },
  { left: "49%", w: 6, h: 9,  color: "#64B5F6", delay: "0.4s", dur: "6.5s" },
  { left: "60%", w: 5, h: 8,  color: "#F6B40E", delay: "3.2s", dur: "5.1s" },
  { left: "71%", w: 7, h: 10, color: "#FFFFFF", delay: "1.2s", dur: "6.0s" },
  { left: "82%", w: 6, h: 9,  color: "#64B5F6", delay: "2.4s", dur: "4.7s" },
  { left: "92%", w: 5, h: 8,  color: "#F6B40E", delay: "0.9s", dur: "6.3s" },
];

function FinalsConfetti() {
  return (
    <div className="bap-papelitos" aria-hidden="true">
      {FINALS_CONFETTI.map((p, i) => (
        <span key={i} style={{
          position: "absolute", top: 0, left: p.left,
          width: p.w, height: p.h, background: p.color,
          borderRadius: 1, animationDelay: p.delay, animationDuration: p.dur,
...
// Graduation cap — the glyph that replaces the sun/moon in the greeting
// strip during finals season. Rendered white/celeste to sit on the
// finals gradient (gold stays exclusive to the Mundial Sol de Mayo).
function GradCapIcon({ size = 64 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M32 14 L60 24 L32 34 L4 24 Z" fill="#FFFFFF" stroke="#E3F2FD" strokeWidth="1.5" strokeLinejoin="round" />
      <path d="M16 29 L16 41 C16 45 24 48 32 48 C40 48 48 45 48 41 L48 29" fill="none" stroke="#FFFFFF" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      <line x1="60" y1="24" x2="60" y2="42" stroke="#FFFFFF" strokeWidth="2.5" strokeLinecap="round" />
      <circle cx="60" cy="44" r="3" fill="#64B5F6" stroke="#FFFFFF" strokeWidth="1.5" />
    </svg>
  );
}
...
function TodayView({ data, onJumpToTab, profile, currentUser, onRefreshData, prompts, onOpenPrompt }) {
  ...
  // Finals-week morale mask. Cohort-wide; gated on the Settings window.
  // Mundial takes visual precedence on the rare overlap day (it's the
  // more time-critical, date-specific treatment), so the finals skin and
  // hero only apply when Argentina isn't also playing today.
  const finalsSeason = isFinalsSeason(data, todayStr) && !argGame;
  ...
  // Finals season puts on a confident BAP-blue → deep Pep-blue skin
  // (distinct from the time-of-day gradient and the celeste Mundial one).
  const finalsGradient = "linear-gradient(135deg, #64B5F6 0%, #0057B8 45%, #00205B 100%)";
  const greetingStrip = (
    <div style={{
      background: argGame ? argGradient : finalsSeason ? finalsGradient : gradient, color: "#FFFFFF", borderRadius: 16,
      padding: "20px 20px 18px", marginBottom: 14, position: "relative",
      overflow: "hidden", boxShadow: "0 4px 16px rgba(0, 32, 91, 0.18)",
    }}>
      <div className={(argGame || (isDayHour && !finalsSeason)) ? "bap-sun-rotate" : ""} style={{
        position: "absolute", top: -10, right: -10, opacity: argGame ? 0.36 : finalsSeason ? 0.34 : 0.3,
        transform: "scale(1.7)", transformOrigin: "center",
      }}>
        {argGame ? <SolDeMayoIcon size={64} /> : finalsSeason ? <GradCapIcon size={64} /> : isDayHour ? <SunIcon size={64} /> : <MoonIcon size={64} />}
      </div>
      {argGame && <Papelitos />}
      {finalsSeason && <FinalsConfetti />}
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        <div lang="es" style={{
          fontFamily: "'DM Mono', monospace", fontSize: 10, textTransform: "uppercase",
          letterSpacing: 2, color: (argGame || finalsSeason) ? "#FFFFFF" : C.bapBlue, marginBottom: 4,
        }}>{argGame ? "¡Hoy juega Argentina! / Game day" : finalsSeason ? "¡Semana de finales! / Finals week" : "Hoy / Today"}</div>
        <div lang="es" style={{
          fontFamily: "'EB Garamond', serif", fontSize: 26, fontWeight: 700,
          lineHeight: 1.05, letterSpacing: -0.4,
          textShadow: (argGame || finalsSeason) ? "0 1px 3px rgba(0, 32, 91, 0.30)" : "none",
        }}>{(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FinalsGoodLuckTile data={data} todayStr={todayStr} onJumpToTab={onJumpToTab} />
...
function WeeklyOverviewView({ data, profile }) {
  ...
  if (wbp === "rail") {
    ...
    return (
      ...
      // same negative-margin pattern as Local Places and Calendar. Height is
      // measured (useFillHeight) rather than guessed: the measured top already
      // accounts for the SectionTitle AND the sub-pill row + (optional)
      // FinalsCard that ScheduleView renders above this component — chrome the
      // old calc() couldn't see and had to absorb into a minHeight backstop.
      <div ref={fillRef} style={{
        display: "flex",
        margin: "-20px -16px -24px",
        height: fillHeight ?? "calc(100vh - 96px)",
        minHeight: 420,
      }}>
...
function CalendarView({ data }) {
  ...
  // ── Rail 2-column event list (landscape iPad only) ───────────────────
  // No detail pane (follow-up B, option A): most calendar events carry
  // little detail, so a master-detail split left a quiet right pane. Instead
  // the extra width is spent on a denser, RICHER list — events flow into two
  // columns of full cards (date, title, time, description, address,
  // add-to-calendar) within each month, with the month header and the
  // "Hoy · Today" divider spanning both columns. This also surfaces the
  // description/time/address that the compact phone + non-rail cards don't
  // show. Filter pills and the today-anchor scroll behave as before. The
  // non-rail path below is byte-identical.
  if (cbp === "rail") {
    const filterPills = (
    ...
    return (
      // Single scrollable column that escapes the App's inner padding (20px
      // top, 16px sides, 24px bottom) with the same negative-margin pattern
      // the other rail views use. Height is measured (useFillHeight) rather
      // than a guessed calc(), so the fit is exact regardless of the
      // SectionTitle's actual rendered height.
      <div ref={fillRef} style={{
        margin: "-20px -16px -24px",
        height: fillHeight ?? "calc(100vh - 96px)",
        minHeight: 420,
        ...
        {filterPills}
        {!hasEvents ? (
          <div style={{ padding: "44px 0", textAlign: "center", fontFamily: "'EB Garamond', serif", fontStyle: "italic", fontSize: 16, color: C.mountain }}>
          ...
          Object.entries(grouped).map(([monthKey, monthEvents]) => {
            ...
            return (
              <div key={monthKey} style={{ marginBottom: 26 }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, textTransform: "uppercase", letterSpacing: 1.5, color: C.mountain, marginBottom: 12, paddingBottom: 6, borderBottom: `1px solid ${C.fog}` }}>{monthName}</div>
                {/* Two columns; month header + today divider span both. */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignItems: "start" }}>
                  {monthEvents.map((e, i) => {
                    const s = EVENT_STYLES[e.type] || EVENT_STYLES.academic;
                    const isMulti = e.end_date && e.end_date > e.date;
                    const dateLabel = isMulti ? `${formatDate(e.date)} – ${formatDate(e.end_date)}` : formatDate(e.date);
                      ...
                      <Fragment key={i}>
                        {isFirstNotPast && (
                          <div ref={todayDividerRef} style={{
                            gridColumn: "1 / -1",
                            display: "flex", alignItems: "center", gap: 10,
                            margin: "2px 0", scrollMarginTop: 12,
                          }}>
                            <div style={{ flex: 1, height: 1, background: C.bapBlue }} />
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 500, color: C.bapBlue, letterSpacing: 1.2, textTransform: "uppercase", padding: "3px 10px", borderRadius: 12, background: C.ice, border: `1px solid ${C.bapBlue}` }}>Hoy · Today</div>
                            <div style={{ flex: 1, height: 1, background: C.bapBlue }} />
                          </div>
                        )}
                        <div style={{
                          background: s.bg, borderRadius: 10, padding: "12px 15px",
                          borderLeft: `3px solid ${s.border}`, opacity: isPast ? 0.6 : 1,
                          boxSizing: "border-box",
                        }}>
                          {/* Date row + type badge */}
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, marginBottom: 5 }}>
                            <div style={{ display: "flex", alignItems: "baseline", gap: 8, minWidth: 0 }}>
                              <span style={{ fontFamily: "'EB Garamond', serif", fontSize: 15, fontWeight: 700, color: C.pepBlack, whiteSpace: "nowrap" }}>{dateLabel}</span>
                              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10.5, color: C.mountain }}>{dayOfWeek}</span>
                            </div>
                            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, fontWeight: 600, color: C.mountain, textTransform: "uppercase", letterSpacing: 1, whiteSpace: "nowrap", flexShrink: 0 }}>
                              {e.type === "mundial" ? "⚽ " : ""}{s.label || e.type}
                            </span>
                          </div>
                          {/* Title */}
                          <div style={{ fontFamily: "'EB Garamond', serif", fontWeight: 700, fontSize: 16, color: C.pepBlue, lineHeight: 1.25 }}>
                            {e.type === "mundial" && <span style={{ marginRight: 5 }}>⚽</span>}{e.title}
                          </div>
                          {/* N-days pill for multi-day */}
                          {isMulti && (
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10.5, color: C.mountain, marginTop: 4 }}>
                              {countDays(e.date, e.end_date)} days
                            </div>
                          )}
                          {/* Time */}
                          {timeLabel && (
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, color: C.ocean, marginTop: 4 }}>{timeLabel}</div>
                          )}
                          {/* Description */}
                          {e.description && (
                            <div style={{ fontFamily: "'Roboto', sans-serif", fontSize: 13.5, color: C.mountain, lineHeight: 1.55, marginTop: 7, whiteSpace: "pre-line" }}>{e.description}</div>
                          )}
                          {/* Address (rare on calendar events, but supported) */}
                          {e.address && (
                            <div style={{ fontSize: 13, color: C.mountain, marginTop: 7, fontFamily: "'Roboto', sans-serif" }}>
...
// Good-luck morale hero shown on the Today tab during finals season
// (gated by isFinalsSeason — the program-wide window from Settings).
// Cohort-wide and personalization-independent: every student sees a
// warm "good luck" message the day finals start through the day they
// end. A grad-cap watermark + Pep-Blue academic accent (distinct from
// the cultural Mundial tile). Tapping jumps to Schedule, where the
// per-student <FinalsCard> shows the actual exam slots. Renders null
// outside the window so the Today layout stays clean the rest of term.
function FinalsGoodLuckTile({ data, todayStr, onJumpToTab }) {
  if (!isFinalsSeason(data, todayStr)) return null;

  return (
    <button
      onClick={() => { if (onJumpToTab) onJumpToTab("schedule"); }}
      className="bap-press"
      aria-label="¡Mucha suerte en los finales! / Good luck on finals!"
      style={{
        position: "relative", overflow: "hidden",
        background: `linear-gradient(135deg, ${C.white} 0%, ${C.ice} 100%)`,
        border: "1px solid #9FC4E8", borderRadius: 14,
        padding: "13px 15px 15px", marginBottom: 14, cursor: "pointer",
        width: "100%", textAlign: "left", font: "inherit", display: "block",
        boxShadow: "0 3px 12px rgba(0, 87, 184, 0.10)",
      }}
    >
      <div aria-hidden="true" style={{
        position: "absolute", right: 10, bottom: -14, zIndex: 0,
        opacity: 0.12, pointerEvents: "none", transform: "rotate(-8deg)",
      }}>
        <GradCapIcon size={88} />
      </div>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div lang="es" style={{
            fontFamily: "'DM Mono', monospace", fontSize: 10, textTransform: "uppercase",
            letterSpacing: 1.4, color: C.pepBlue,
            display: "inline-flex", alignItems: "center", gap: 5,
          }}>
            <span aria-hidden="true">🎓</span> Semana de finales · Finals week
          </div>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: C.pepBlue }}>
            Ver →
          </span>
        </div>
        ...
        }}>¡Mucha suerte en los finales!</div>

        <div style={{
          fontFamily: "'EB Garamond', serif", fontStyle: "italic",
          fontSize: 15, color: C.ocean, marginTop: 2,
        }}>Good luck on finals!</div>

        <div lang="es" style={{
          fontFamily: "'Roboto', sans-serif", fontSize: 12.5,
          color: C.mountain, marginTop: 9, lineHeight: 1.4,
        }}>Confiá en lo que estudiaste — un examen a la vez. <span style={{ color: C.stone }}>You've got this.</span></div>
      </div>
    </button>
  );
}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
            ...
            <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 2, margin: "-8px -6px -8px 0" }}>
              ...
              <button
                onClick={(e) => { e.stopPropagation(); onToggleSave(place.place_id); }}
                className="bap-press"
...
function LocalView({ data, initialSub, resetSignal, places = [], savedPlaces = [], onToggleSavePlace, onSubChange, onOpenSuggest, onRegisterBack }) {
  ...
  // Rail master-detail: the place whose left-list card has been tapped. Pans
  // and highlights its pin on the always-open map (a no-op for places without
  // coordinates). Cleared when the category/filter changes so a stale ring
  // doesn't linger. Only consumed in the rail two-pane layout.
  const [focusedPlaceId, setFocusedPlaceId] = useState(null);
  useEffect(() => { setFocusedPlaceId(null); }, [placesFilter]);
        ...
        if (lbp === "rail") {
              ...
              {display.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {display.map((p, i) => {
                    ...
                    const isFocused = focusedPlaceId === p.place_id;
                    return (
                      // Tapping the card body pans/highlights the pin on the
                      // right-pane map. The inner share/save buttons and link
                      // anchors stopPropagation (share/save) or open a link, so
                      // they don't double-fire this. Coordless places have no
                      // pin, so the tap just shows the selection ring.
                      <div
                        key={p.place_id || i}
                        onClick={() => setFocusedPlaceId(p.place_id)}
                        style={{
                          cursor: hasCoords ? "pointer" : "default",
                          borderRadius: 10,
                          boxShadow: isFocused ? `0 0 0 2px ${C.ocean}` : "none",
          ...
          return (
            ...
            <div ref={fillRef} style={{
              ...
              margin: "-20px -16px -24px",
              // Height is measured (useFillHeight) rather than a guessed calc(),
              // so the two panes fit exactly; the map's ResizeObserver still
              // remeasures Leaflet when this height changes on rotate.
              height: fillHeight ?? "calc(100vh - 96px)",
              minHeight: 420,
            }}>
...
// ─── Fill-remaining-height hook (rail master-detail / full-height views) ───
// The rail master-detail blocks (Local Places, Calendar, Schedule Weekly
// Overview) need to fill from their own top down to the bottom of the
// viewport. They used to do that with a guessed calc() — `100vh - 44px (the
// inner padding) - 52px (the SectionTitle) - the safe-area insets` — which
// is brittle: the 52px is just an estimate of the SectionTitle's rendered
// height, so the fit lands slightly off on some iPad sizes. This measures
// the real available space instead. Attach the returned callback ref to the
// block's wrapper; on mount (and on every resize / orientation change) it
// reads the wrapper's actual viewport top via getBoundingClientRect and
// returns `innerHeight - top - bottomGap`, so the height is correct no matter
// what the SectionTitle or chrome actually measure. The callback ref fires
// during commit (before paint), so there's no first-frame flash. Floored at
// 420px so a freak measurement can't collapse the pane. `bottomGap` is the
// breathing room left below the pane (rail has no bottom nav, so a small gap).
function useFillHeight(bottomGap = 16) {
  ...
  const measure = useCallback(() => {
    ...
    setHeight(Math.max(420, Math.round(window.innerHeight - top - bottomGap)));
  }, [bottomGap]);
  // Callback ref: measure the moment the node attaches (handles entering rail
  // or rotating into it without a stale value).
  ...
  return (
    <div style={{
      ...
      {/* Bottom nav — in NORMAL FLOW (a flex child of the 100dvh column),
          not position:fixed. iOS won't reliably paint a fixed element (or
          its box-shadow) into the home-indicator safe area, so a fixed
      ...
      {bp !== "rail" && <nav ref={navRef} aria-label="Secciones / Sections" style={{
        ...
        padding: "8px 0 var(--bap-nav-pad-bottom)",
      }}>
        {TABS.map((t) => {
          const active = tab === t.key;
