...
const DEFAULT_DATA = {
  ...
  classes: [
    { code: "IES 300", title: "Argentine History & Society", professor: "García", honorific: "Prof.", firstname: "Ana", daysRaw: "MW", days: ["Mon", "Wed"], time: "9:00–10:30", location: "Classroom A", color: "#0057B8", email: "" },
    { code: "SPA 201", title: "Intermediate Spanish II", professor: "Martínez", honorific: "Prof.", firstname: "Carlos", daysRaw: "MTR", days: ["Mon", "Tue", "Thu"], time: "11:00–12:00", location: "Classroom B", color: "#64B5F6", email: "" },
    { code: "REL 100", title: "The Way of Jesus", professor: "Smith", honorific: "Dr.", firstname: "John", daysRaw: "TR", days: ["Tue", "Thu"], time: "14:00–15:30", location: "Classroom A", color: "#425563", email: "" },
    { code: "COM 300", title: "Intercultural Communication", professor: "Álvarez", honorific: "Prof.", firstname: "María", daysRaw: "WF", days: ["Wed", "Fri"], time: "14:00–15:30", location: "Classroom C", color: "#6CACE4", email: "" },
    { code: "ART 280", title: "Tango & Argentine Arts", professor: "Reyes", honorific: "Prof.", firstname: "Lucía", daysRaw: "F", days: ["Fri"], time: "10:00–12:30", location: "Studio", color: "#E35205", email: "" },
  ],
  calendarEvents: [
...
async function fetchAllData() {
  ...
  return {
    ...
    classes: classesRaw.filter(r => r.code).map((r) => ({
      ...
      daysRaw: r.days ? r.days.trim() : "",
      days: parseDayCodes(r.days),
      time: r.time.trim(),
...
// Map single-letter day codes (M T W R F) to three-letter abbreviations
const DAY_CODE_MAP = { M: "Mon", T: "Tue", W: "Wed", R: "Thu", F: "Fri" };

function parseDayCodes(raw) {
  if (!raw) return [];
  const s = raw.trim();
  // If it already contains three-letter abbreviations (comma-separated), use the old format
  if (/\b(Mon|Tue|Wed|Thu|Fri)\b/.test(s)) {
    return s.split(",").map((d) => d.trim());
  }
  // Otherwise treat each character as a single-letter code
  return [...s].map((ch) => DAY_CODE_MAP[ch.toUpperCase()]).filter(Boolean);
}
                ...
                <div style={{ fontSize: 14, color: C.mountain, fontFamily: "'Roboto', sans-serif", lineHeight: 1.7 }}>
                  ...
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6, verticalAlign: "middle" }}>
                    <span style={{ display: "inline-flex", gap: 3 }}>
                      {["M","T","W","R","F"].map((letter) => {
                        const active = c.daysRaw && c.daysRaw.includes(letter);
                        return (
                          <span key={letter} style={{
                            display: "inline-flex", alignItems: "center", justifyContent: "center",
                            width: 22, height: 22, borderRadius: "50%", fontSize: 11, fontWeight: 600,
                            fontFamily: "'DM Mono', monospace",
                            background: active ? c.color : C.parchment,
                            color: active ? C.white : C.fog,
                          }}>{letter}</span>
                        );
                      })}
                    </span>
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, color: C.stone }}> · {c.time}</span>
                  </span><br />
                  {c.location}
                </div>
                {c.email && (
