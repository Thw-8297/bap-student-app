...
const LOCAL_SECTIONS = [
  ...
  { key: "explore",  en: "Explore BA", es: "Lugares para descubrir", Icon: ObeliscoIcon,    accent: C.ocean },
  ...
  if (items.length === 0 && suppressEmptyForHoliday && !hasInlineMeals) {
    ...
    activityCard = (
      <div style={{
        ...
        {onJumpToTab && (
          <button
            onClick={() => onJumpToTab("local", "explore")}
            className="bap-press"
            style={{
...
// ─── Local ───
function LocalView({ data, initialSub }) {
  ...
  // Off-state ordering is left exactly as it was.
  ...
  const displayExplore = nearMe && userLoc ? sortByDistance(filteredExplore, userLoc) : filteredExplore;
  ...
  return (
    <div>
      ...
      {sub === "explore" && (
        <div>
          {anyCoords(data.explore || []) && nearMeControl}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {displayExplore.map((p, i) => {
              const cap = distanceCaption(p);
              return (
              <Card key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
...
export default function App() {
  ...
  // button → "explore") and consumed by <LocalView>'s initialSub; cleared
      ...
      <div style={{ flex: 1, minHeight: 0, padding: "20px 16px 100px", overflowY: "auto", WebkitOverflowScrolling: "touch", overscrollBehaviorY: "contain" }}>
        {status === "loading" ? (
          ...
          <>
            ...
            {tab === "local" && <LocalView data={data} initialSub={localInitialSub} />}
