...
//
// "Blended 11": the daily-life buckets students will submit (café,
// restaurant, nightlife, fitness, study) sit alongside the sightseeing
// buckets the seed content needs (culture, theater, sights, markets,
// neighborhood). Filter pills are present-gated, so empty buckets stay
// hidden until rows exist — a student-life category simply doesn't show
// until someone submits one. The six seed-present categories (culture,
// theater, sights, markets, neighborhood, outdoors) carry the most
// distinct colors since they're what renders at launch.
const PLACE_CATEGORIES = {
  ...
  culture:      { label: "Culture",      es: "Cultura",         color: C.sky,       Icon: PictureFrameIcon },
  theater:      { label: "Theater",      es: "Teatro",          color: C.pepBlue,   Icon: TheaterMaskIcon },
  sights:       { label: "Sights",       es: "Lugares ícono",   color: C.ocean,     Icon: ObeliscoIcon },
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 13, color: C.mountain, fontFamily: "'Roboto', sans-serif", lineHeight: 1.6, marginTop: 3, whiteSpace: "pre-line" }}>
            ...
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10.5, textTransform: "uppercase", letterSpacing: 0.8, color: meta.color }}>
...
// ─── Local ───
function LocalView({ data, initialSub, places = [], savedPlaces = [], onToggleSavePlace }) {
  ...
  const [placesFilter, setPlacesFilter] = useState("all"); // "all" | <category key> | "saved"
  ...
  return (
    <div>
      ...
      {sub === "places" && (() => {
        ...
        // Pills follow PLACE_CATEGORIES declaration order, limited to what's present.
        const present = new Set(allPlaces.map((p) => p._cat).filter(Boolean));
        const orderedCats = Object.keys(PLACE_CATEGORIES).filter((k) => k !== "other" && present.has(k));
        ...
        return (
          <div>
            ...
            {(orderedCats.length > 0 || hasSaved) && (
              <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
                <FilterPill active={placesFilter === "all"} onClick={() => setPlacesFilter("all")}>All</FilterPill>
                {orderedCats.map((k) => (
                  <FilterPill key={k} active={placesFilter === k} onClick={() => setPlacesFilter(k)}>{PLACE_CATEGORIES[k].label}</FilterPill>
                ))}
                {hasSaved && (
                  <FilterPill active={placesFilter === "saved"} onClick={() => setPlacesFilter("saved")}>♥ Saved</FilterPill>
                )}
              </div>
            )}
            {display.length === 0 ? (
...
export default function App() {
  ...
  const jumpToTab = useCallback((tabKey, sub = null) => {
    if (tabKey === "local") setLocalInitialSub(sub);
    setTab(tabKey);
  }, []);
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => { if (t.key === "local") setLocalInitialSub(null); setTab(t.key); }}
              className="bap-press"
              style={{
