...
The token is stored as a plain string at the localStorage key `bap-cohort-token`. No version field; rotation is handled by server-side validation rather than a client-side schema. Profile and data cache live at separate keys (`bap-profile`, `bap-app-cache`), so wiping the cohort token doesn't disturb a student's enrolled-classes selections or their cached content.
