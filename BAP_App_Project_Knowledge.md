...
- **Bolder color confidence.** The palette is currently used cautiously: lots of white cards, fog borders, one gradient in the header. Pushing BAP Blue into more surfaces (today's class card background, section header bands) and treating white as the secondary would land closer to a contemporary editorial app feel. The most identity-shifting change in this list; benefits from a pre-build mockup pass before implementation.
...
- **More mini-illustrations in active rotation.** The library now includes mate (Weekly Overview empty days, Today empty state), sun (Today greeting day-time), moon (Today greeting night), weather glyphs (Today quick-stats), colectivo and palm (Apps section dividers). Obelisco, tango shoe, and Río de la Plata wave are defined but not yet wired into views; each is one render line away from going live (e.g., obelisco for an Explore view header, tango for a cultural-events badge).
