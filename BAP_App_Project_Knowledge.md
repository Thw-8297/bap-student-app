...
- **Places (student-contributable BA recommendations).** *Designed 2026-06-02; not yet built.* Evolves the existing Explore BA sub-tab into a categorized, student-contributable directory of Buenos Aires spots (cafés, restaurants, parks, gyms, climbing walls, study spots, etc.). Today's curated Explore BA rows become the seed data. The novel part is a **submit → queue → vet → publish** pipeline: a student suggests a place, it lands as `pending`, a staff member approves it in-app, and it then renders for everyone.
  ...
  - **Map / spatial features — phased.** *Direction set 2026-06-07.* Two distinct things were conflated under "map" and are now separated, because the high-value half needs no map at all:
    ...
    - **Phase 2 — embedded interactive map (RECOMMENDED: Leaflet, online-only; *recommendation set 2026-06-09, not yet built*).** Now that Places has shipped both stages (read path + student submission + vetting), the coordinate seed from Phase 1 is in place and the data will start to accumulate enough pins that pan/zoom browsing beats a scroll-list. The recommended build:
      ...
      - **Open questions to settle during the spike:** (1) confirm CARTO's no-key raster endpoint + attribution on a real device; (2) an **iOS-PWA gesture test** (Leaflet touch/drag vs. the page scroll container and the bottom-nav) on an installed home-screen PWA, not just desktop Safari; (3) whether pin **clustering** is needed at all — at cohort volume (dozens of pins max) probably skip `leaflet.markercluster` to avoid a second dependency, revisit only if a category ever gets crowded.
      - **Process.** This is a "bigger change" per the workflow rules: it adds a dependency (needs explicit sign-off), is multi-component UI, and warrants a throwaway phone spike before building for keeps. Build on a branch, mockup + spike first, then merge deliberately. No `CACHE_VERSION` bump (data shape is untouched; the map only reads existing `lat`/`lng`), no Apps Script change, no sheet-schema change.
