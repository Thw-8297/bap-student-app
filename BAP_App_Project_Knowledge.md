...
- **Places (student-contributable BA recommendations).** *Designed 2026-06-02; not yet built.* Evolves the existing Explore BA sub-tab into a categorized, student-contributable directory of Buenos Aires spots (cafés, restaurants, parks, gyms, climbing walls, study spots, etc.). Today's curated Explore BA rows become the seed data. The novel part is a **submit → queue → vet → publish** pipeline: a student suggests a place, it lands as `pending`, a staff member approves it in-app, and it then renders for everyone.
  ...
  - **Map / spatial features — phased.** *Direction set 2026-06-07.* Two distinct things were conflated under "map" and are now separated, because the high-value half needs no map at all:
    ...
    - **Phase 2 — embedded interactive map (deferred; revisit once Places has volume).** A real pin map only pays off when Places grows past a handful of curated rows to dozens of student spots, where pan/zoom browsing beats a scroll-list. Options, cheapest-first: **(a) Leaflet + a tile provider** — Leaflet core is ~42 KB gzipped JS + CSS (meaningful against the current ~91 KB gzipped bundle); OpenStreetMap's public tile server is free/no-key but its usage policy discourages app traffic, so the "correct" basemap is a free-tier tile CDN (Carto, Stadia, or Protomaps — some need a key). **(b) MapLibre GL** — vector tiles, smoother but heavier (~200 KB) and more GPU-dependent on older phones. **(c) Mapbox / Google Maps JS** — paid/keyed, best polish, rejected unless billing is acceptable. **The hard constraint is the offline-on-the-subte promise:** map tiles require network, and SW-caching a useful zoom range across all of CABA is many MB, so the map degrades offline to pins on a blank canvas — meaning the **list view must remain the real offline experience** and the map is an online-only enhancement. Also needs an iOS-PWA gesture-interplay test (Leaflet touch handling vs. the `<BottomSheet>` drag) before committing. Treat as a "bigger change": branch, mockup, and a throwaway spike on a phone to judge the bundle/offline tradeoff for real before building it for keeps.
