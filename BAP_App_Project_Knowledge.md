...
- Opens to a **category hub** (added 2026-06-08): five large, stacked, full-width buttons in this order — This Week, Explore BA, Healthcare, Churches, Apps — each with a glyph in a tinted Ice container, an EB Garamond English label + Roboto Spanish gloss, a 4 px left accent stripe in the category's color, and a → chevron. The hub previews no listings; tapping a button drills into that category's listing, and a "Volver / Back" header (DM Mono pill + the EB Garamond section title) returns to the hub. The hub replaced the previous two-row pill selector. It's driven by the module-scope `LOCAL_SECTIONS` config array (`{ key, en, es, Icon, accent }`) — single source of truth, so renaming/replacing "Explore BA" with the roadmap **Places** feature is a one-line edit, and the per-category listing screen becomes the natural home for Places' future "+ Suggest a place" entry point. Glyphs: `SparkleIcon` (This Week), `ObeliscoIcon` (Explore BA — previously defined-but-unused, now in service), and three new inline-SVGs `HealthCrossIcon` / `ChurchIcon` / `AppGridIcon`.
