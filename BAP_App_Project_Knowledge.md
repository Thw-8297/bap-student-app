...
**Tier 6 — feature ideas surfaced by the review (fold into Future ideas as they mature)**

23. **Search across FAQ and Places.** 43+ places and a growing FAQ make scan-only retrieval slow; a simple client-side filter input would do.
24. **Unread state for announcements.** No badge or "new since you last looked" affordance, so a student who skips Today for a few days can miss an urgent banner's entire window. (Distinct from push notifications, already on the roadmap below — this is a passive in-app cue.)
25. **Add-to-calendar (.ics) export** for calendar events and finals.
26. **Saved places: count and share.** The ♥ list has no count surfaced anywhere and no way to share a place with a friend.
