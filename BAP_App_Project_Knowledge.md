...
**Tier 5 — UX consistency and accessibility**

18. **Client-side required-field validation in `<PromptForm>`.** It currently submits straight to the Apps Script and waits a 2–4 s round trip to learn "Falta completar: X," while `<PlaceSubmitForm>` validates client-side — and the form already knows each field's `required` flag. Make the two consistent.
19. **A non-touch refresh affordance on Today.** Pull-to-refresh is touch-only, but the documented Director workflow ("pull down on Today after editing the sheet") doesn't exist on a laptop. A small refresh control on Today serves desktop, keyboard, and assistive-tech users.
20. **Bilingualize the wayfinding outliers.** The ES/EN primary-language direction flips by surface with no clear rule; the worst outliers are the status pill (six EN-only labels — "Saved version (offline)" is exactly the message a student on the subte needs in either language) and the ProfileModal body (essentially EN-only). Also: `<html lang="en">` on a half-Spanish app gives VoiceOver English pronunciation of Spanish copy — add `lang="es"` to Spanish spans (or at the surface level), and standardize aria-label language (currently a mix of EN-only and bilingual).
21. **Modal and nav semantics.** No focus trap, no Escape-to-close, no focus return in any sheet or modal; `<ProfileModal>` and both Director overlays lack `role="dialog"` / `aria-modal` entirely (BottomSheet has them). The bottom nav is a plain `div` with no `<nav>` landmark or `aria-current` on the active tab; filter pills carry no `aria-pressed`; the FAQ accordion lacks `aria-expanded`; the status pill has no `aria-live`, so the offline transition is never announced. Three `role="button"` divs (the "My classes only" pill, `<TodayFinalsTile>`, `<EventsTodayTile>`) need `tabIndex` + key handlers or conversion to real buttons. Mostly matters for iPad-with-keyboard and desktop Director use.
22. **Replace `window.confirm` for sign-out / profile reset** with the app's own confirm treatment — functional today, but visually jarring against the otherwise polished modal system, and the bilingual text renders as one cramped native dialog. Low priority.
