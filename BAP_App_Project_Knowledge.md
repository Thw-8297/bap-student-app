...
2. **✅ Re-enable pinch-zoom.** The viewport meta in `index.html` carried `maximum-scale=1.0, user-scalable=no`. Android Chrome honors it, so a student couldn't zoom an address or the 10 px DM Mono labels; it was also a hard WCAG 1.4.4 failure. Both directives removed → viewport is now just `width=device-width, initial-scale=1.0`. **Note:** `viewport-fit=cover` was briefly added alongside the removal, **reverted the same day** when it regressed the iOS safe areas (gear under the battery, dead white strip at the bottom), then **re-added in 2026-06-09h together with the full `env(safe-area-inset-*)` padding pass** — see Tier 4 #12, which is now shipped.
...
12. **✅ iOS safe-area insets (shipped 2026-06-09h).** `viewport-fit=cover` is back in the viewport meta, and `env(safe-area-inset-*)` is now consumed everywhere a fixed or full-screen element meets a screen edge: the main header top padding + the absolutely-positioned gear's `top` (status bar no longer covers the gear); the bottom nav `padding-bottom` (`calc(8px + inset)`, replacing the flat `16px`), the content area `padding-bottom` (`calc(100px + inset)` so the last card clears the taller nav), the Places FAB (`bottom: calc(90px + inset)`) and `<PlaceToast>` (`bottom: calc(84px + inset)`); the three full-screen overlay headers (Settings + both Director views, identical block, done with one `replace_all`) and their footers; and the `<BottomSheet>` body (`calc(28px + inset)` so the last control clears the home indicator). **Lesson learned the hard way:** a first attempt added `viewport-fit=cover` alone without the padding and regressed both ends (gear under the battery, dead white strip at the bottom) — cover makes the insets non-zero but they MUST be consumed in the same change. The gates (`<PasscodeGate>` / `<UserGate>`) were intentionally left alone — they're vertically-centered with a full-bleed gradient, so cover is harmless there. Pure layout; no `CACHE_VERSION` bump.
