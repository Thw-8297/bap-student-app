...
2. **✅ Re-enable pinch-zoom.** The viewport meta in `index.html` carried `maximum-scale=1.0, user-scalable=no`. Android Chrome honors it, so a student couldn't zoom an address or the 10 px DM Mono labels; it was also a hard WCAG 1.4.4 failure. Both directives removed; `viewport-fit=cover` added at the same time (a visual no-op until Tier 4 #12 adds the `env(safe-area-inset-*)` padding, but it's the correct end-state declaration and is what makes those insets non-zero on iOS).
...
12. **iOS safe-area insets** — the most visible polish gap on the devices students actually carry. With `black-translucent` status bar and no `viewport-fit=cover`, the installed PWA on a notched iPhone draws the header under the clock and the home indicator over the FAQ/Contacts nav labels. Add `viewport-fit=cover` to the viewport meta, `padding-top: env(safe-area-inset-top)` on the header, `padding-bottom: calc(16px + env(safe-area-inset-bottom))` on the bottom nav, and matching offsets for the Places FAB and `<PlaceToast>`.
