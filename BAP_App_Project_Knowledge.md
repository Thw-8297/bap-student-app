...
- **mundial**: Championship-gold background (`#FFF8E1`), amber border (`#F9A825`), ⚽ (added 2026-06-01 for World Cup games; the ⚽ soccer ball is prefixed to the event title — the only event type that renders its glyph inline, all others are distinguished by bg/border alone)
...
- Opens to a **category hub** (added 2026-06-08): five large, stacked, full-width buttons in this order — This Week, **Places**, Healthcare, Churches, Apps — each with a glyph in a tinted Ice container, an EB Garamond English label + Roboto Spanish gloss, a 4 px left accent stripe in the category's color, and a → chevron. The hub previews no listings; tapping a button drills into that category's listing, and a "Volver / Back" header (DM Mono pill + the EB Garamond section title) returns to the hub. The hub replaced the previous two-row pill selector. It's driven by the module-scope `LOCAL_SECTIONS` config array (`{ key, en, es, Icon, accent }`) — single source of truth. Glyphs: `SparkleIcon` (This Week), `ObeliscoIcon` (Places), and three new inline-SVGs `HealthCrossIcon` / `ChurchIcon` / `AppGridIcon`.
...
- "Cerca tuyo / Near you" distance sort (added 2026-06-07) on the Explore BA, Healthcare, and Churches sub-tabs: a "📍 Cerca tuyo / Near you" toggle (shown only once a sub-tab has rows with `lat`/`lng` coordinates) re-sorts the list nearest-first using the phone's location and shows a small distance caption per located card. One permission prompt is shared across the three tabs; toggling off restores the default order; declining the permission shows an inline note and keeps the default order. No map, on-device only, offline-safe.
...
- **`LOCAL_SECTIONS`** — module-scope config array of `{ key, en, es, Icon, accent }` for the five Local-tab category-hub buttons (This Week, Explore BA, Healthcare, Churches, Apps). Single source of truth for the hub's order, labels, glyphs, and accent colors; the roadmap **Places** feature replaces the `explore` entry here. Icon components are hoisted function declarations, so referencing them above their definitions is fine.
- **`<LocalView data initialSub />`** — the Local tab. When `sub` is `null` it renders the category hub (one stacked button per `LOCAL_SECTIONS` entry); when `sub` is set it renders a "Volver / Back" header (`setSub(null)` returns to the hub) above that category's listing. `initialSub` (default `null` → hub) lets Today's deep-links land directly on a listing; `<App>` supplies it via `localInitialSub` state set by the `jumpToTab(tabKey, sub)` wrapper and cleared on a normal Local-nav tap. All listing sub-views (`<EventsView>`, health/churches/explore/apps cards, the "Cerca tuyo / Near you" sort) are unchanged by the hub refactor.
...
25. **Add-to-calendar (.ics) export** for calendar events and finals.
