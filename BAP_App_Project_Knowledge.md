...
**Tier 2 — security hardening, one `AuthCode.gs` re-deploy (no client change, no `CACHE_VERSION` bump)** — ✅ code shipped 2026-06-09; **requires a manual `AuthCode.gs` re-deploy to take effect** (Apps Script editor → Deploy → Manage deployments → pencil → New version)
