...
**Tier 2 — security hardening, one `AuthCode.gs` re-deploy (no client change, no `CACHE_VERSION` bump)** — ✅ **fully shipped:** code 2026-06-09, and the manual `AuthCode.gs` re-deploy was completed by the Director (confirmed 2026-06-09), so the formula-injection guard + identify throttle are live.
...
5. **Lower priority, same deploy if convenient:** (a) `identify` / `prompts` / `admin_responses` / `admin_places` send token + CWID + birthday in GET query strings, which land in Apps Script execution logs; the POST paths already keep credentials in the body — move the GETs to POST `text/plain` to match. (b) `handleVetPlace` does an unconditional in-place status write with no state machine and overwrites `vetted_by` (no history); consider an append-only vetting-log tab and guarding the `rejected` → `approved` transition.
