...
**Tier 1 — trivial config fixes, ship immediately**
...
2. **Re-enable pinch-zoom.** The viewport meta in `index.html` carries `maximum-scale=1.0, user-scalable=no`. Android Chrome honors it, so a student can't zoom an address or the 10 px DM Mono labels; it's also a hard WCAG 1.4.4 failure. Delete both directives — the fixed 480 px column gains nothing from suppressing zoom.

**Tier 2 — security hardening, one `AuthCode.gs` re-deploy (no client change, no `CACHE_VERSION` bump)**
