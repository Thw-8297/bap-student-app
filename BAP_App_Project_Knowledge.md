...
2. **✅ Re-enable pinch-zoom.** The viewport meta in `index.html` carried `maximum-scale=1.0, user-scalable=no`. Android Chrome honors it, so a student couldn't zoom an address or the 10 px DM Mono labels; it was also a hard WCAG 1.4.4 failure. Both directives removed → viewport is now just `width=device-width, initial-scale=1.0`. **Note:** `viewport-fit=cover` was briefly added alongside the removal but **reverted the same day** — combined with the `black-translucent` status bar it pushed content edge-to-edge into both safe areas with no padding to compensate, so the settings gear landed under the iPhone status bar (battery) and the bottom nav showed a dead white strip over the home-indicator area. `viewport-fit=cover` must ship **together with** the `env(safe-area-inset-*)` padding in Tier 4 #12 (header, nav, FAB, toast), not on its own — see that item.
...
12. **iOS safe-area insets** — the most visible polish gap on the devices students actually carry. With `black-translucent` status bar and no `viewport-fit=cover`, the installed PWA on a notched iPhone draws the header under the clock and the home indicator over the FAQ/Contacts nav labels. **Ship as one atomic change** (a 2026-06-09 attempt to add `viewport-fit=cover` alone, without the padding, regressed both ends — gear under the battery, dead white strip at the bottom — and was reverted; cover makes the insets non-zero but you MUST consume them in the same commit): add `viewport-fit=cover` to the viewport meta, AND `padding-top: calc(16px + env(safe-area-inset-top))` on the header (with the absolutely-positioned gear's `top` offset by the same inset, currently `top: 12`, [App.jsx](src/App.jsx) header block ~10432–10447), `padding-bottom: calc(8px + env(safe-area-inset-bottom))` on the bottom nav (drop the current flat `16px`; the nav is `position: fixed; bottom: 0` ~10519–10524) with the content area's `padding-bottom` bumped from `100px` to `calc(100px + env(safe-area-inset-bottom))` (~10477) so the last card still clears the taller nav, and matching offsets for the Places FAB (~7668) and `<PlaceToast>` (~7088). Verify on a notched device — env() values can't be eyeballed in a desktop preview.
